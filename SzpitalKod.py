...
def make_queries(cursor):
    ...
    if num_que == 8:
        ...
        print(cursor.fetchall())
    if num_que == 9:
        cursor.execute("SELECT Pacjenci.Oddzial_ID, count(Pacjenci.Pacjent_ID) as PacjenciCount, O.Nazwa from Pacjenci inner join Oddzialy O on Pacjenci.Oddzial_ID = O.Oddzial_ID group by Pacjenci.Oddzial_ID having PacjenciCount > (select (select count(*)\
